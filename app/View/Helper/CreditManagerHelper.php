...
class CreditManagerHelper extends AppHelper {
    ...
    public function flash($creditStatus, $settings) 
    {
        ...
        if (isset($creditStatus) && isset($settings)) {
            $message = $this->getStatusMessage($creditStatus, $settings);
            if ($message) {
                $out = '<div id="credit-status" class="message credit-message">'.$message.'</div>';
            }
        }
    ...
    public function getStatusMessage($creditStatus, $programSettings) 
    {
        $out = '';
        ...
        case 'ok':
            ...
            if (0 < $balance && $balance <= 50) {
                $out .= __('Warning only %s credits are available for sending message.', $balance);
            }
