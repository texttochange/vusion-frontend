    ...
    $.each(
        ...
        function (key, elt){
            ...
            */   
            ...
            $("<span style='float: right; padding-right: 13px;'> <span id="+$(elt).attr('name')+">"+$(elt).val().replace(/{.*}/g, '').length+"</span> : <span id='messages'>0 SMS Message(s)</span></span>").insertBefore($(elt));            
            $(elt).keydown(function(){
                    //alert("hi");                   
                    ...
                    $('[id="messages"]').text(messages + ' SMS Message(s)');	                     
            }); 
        });
